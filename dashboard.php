...
$online = [];
try {
    $rows = db()->query("SELECT ua.user_id, ua.name, ua.activity, u.role, u.email,
                TIMESTAMPDIFF(MINUTE, ua.login_at, NOW()) AS mins_online
    ...
    $seen = []; // one line per user; non-admins never see admins; super admin never shown
    foreach ($rows as $r) {
        if (isset($seen[$r['user_id']])) continue;
        if (strtolower(trim((string)($r['email'] ?? ''))) === strtolower(SUPERADMIN_EMAIL)) continue;
  ...
  <div class="rightcol">
    ...
    <div class="panel online-widget">
      ...
      <?php else: foreach ($online as $o): ?>
        <div class="ow-row"><span class="ow-dot"></span><span class="ow-name"><?= e($o['name']) ?></span><span class="ow-meta"><?= online_meta($o['activity'] ?? '') ?></span></div>
      <?php endforeach; endif; ?>
      </div>
    </div>
  </div>
